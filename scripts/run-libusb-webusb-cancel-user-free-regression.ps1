...
$build = [IO.Path]::GetFullPath((Join-Path $repo 'build/libusb-webusb-cancel-user-free-regression'))
...
$module = Join-Path $build 'libusb-webusb-cancel-user-free-regression.js'
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw 'em++ failed compiling the user-free harness.' }
    $link = @('--bind','-s','ASYNCIFY=1','-s','ASSERTIONS=2','-s','MODULARIZE=1','-s','EXPORT_ES6=1','-s','ENVIRONMENT=node',
        '-s','EXPORTED_RUNTIME_METHODS=["ccall"]',
        '-s','EXPORTED_FUNCTIONS=["_webts_libusb_webusb_cancel_user_free_regression"]',
    ...
    & $emxxExecutable @link
    if ($LASTEXITCODE -ne 0) { throw 'em++ failed linking the user-free Node module.' }
} finally { Pop-Location }
$node = Get-Command node -ErrorAction Stop
$runner = Join-Path $repo 'scripts/libusb-webusb-cancel-user-free-regression-runner.mjs'
...
if (-not $process.WaitForExit($TimeoutMilliseconds)) {
    $process.Kill($true)
    throw "User-free regression exceeded ${TimeoutMilliseconds}ms."
}
$stdout = $process.StandardOutput.ReadToEnd()
$stderr = $process.StandardError.ReadToEnd()
if ($process.ExitCode -ne 0) { throw "User-free regression failed: stdout=$stdout stderr=$stderr" }
if (-not [string]::IsNullOrWhiteSpace($stderr)) { throw "User-free regression stderr: $stderr" }
$report = $stdout.Trim() | ConvertFrom-Json
if ($report.diagnostic -ne 'OBSERVED' -or $report.callbackCount -ne 1 -or
    $report.callbackStatus -ne 3 -or $report.eventResult -ne 0 -or
    $report.backendCancelReturn -ne 0 -or $report.freedInCallback -ne $true) {
    throw "Unexpected user-free report: $stdout"
}
