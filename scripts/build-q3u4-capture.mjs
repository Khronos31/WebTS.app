// Builds the PX-Q3U4 TS reception module.
//
// This is the threaded driver build plus the upstream stream data plane. The
// data plane starts one bridge pump std::thread per IT930x, so three threads
// run at once: the driver thread this module creates, and two pumps it does
// not. PTHREAD_POOL_SIZE is 4 so none of them has to wait for a Worker to be
// spawned mid-capture; the driver thread blocks while a pump starts, and the
// main runtime thread is the only one that can spawn it.
//
...
const output = join(root, 'build', 'q3u4-capture');
...
run(emxx, [
  ...
  '-s', 'EXPORTED_FUNCTIONS=["_webts_q3u4_capture_start","_webts_q3u4_capture_poll",' +
    '"_webts_q3u4_capture_join","_webts_q3u4_capture_error_name","_malloc","_free"]',
  '-o', module, ...objects,
]);

const text = readFileSync(module, 'utf8');
for (const required of ['webts_q3u4_capture_start', 'ccall']) {
  if (!text.includes(required)) throw new Error(`q3u4-capture.mjs is missing ${required}`);
}
