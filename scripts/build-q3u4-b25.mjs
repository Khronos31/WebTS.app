// Builds the libaribb25 B_CAS_CARD probe module.
//
...
const output = join(root, 'build', 'q3u4-b25');
...
// b_cas_card.c is C and gets the shim's winscard.h ahead of any system one.
{
  const object = join(variantRoot, 'b_cas_card.o');
  run(emcc, [...COMMON, '-I', join(root, 'native', 'winscard'), '-I', join(b25, 'src'),
    '-c', join(b25, 'src', 'b_cas_card.c'), '-o', object]);
  objects.push(object);
}
...
for (const source of [join(root, 'native', 'q3u4-b25-probe.cpp'), ...cxxShims,
  ...upstreamSources]) {
...
const module = join(output, 'q3u4-b25.mjs');
run(emxx, [
  ...
  '-s', 'EXPORTED_FUNCTIONS=["_webts_q3u4_b25_start","_webts_q3u4_b25_poll",' +
    '"_webts_q3u4_b25_join","_webts_q3u4_b25_error_name","_malloc","_free"]',
  '-o', module, ...objects,
]);

const text = readFileSync(module, 'utf8');
for (const required of ['webts_q3u4_b25_start', 'ccall']) {
  if (!text.includes(required)) throw new Error(`q3u4-b25.mjs is missing ${required}`);
}
