...
**PX-Q3U4 以外は、手元に実機が無いまま上流に追従して入れた。**上流も追加機種はすべて「実機未検証」としている（DTV02-1T1S-U、および DTV03A-1TU の 2021-11 以降のロットは「実験的」）。デバイスを動かす部分は上流のコードをそのまま使い、組み立ては上流 px4d と同じ手順にしてある。カードは全機種 IT930x 経由で読む（上流の作り。カードリーダまで届くことを上流が確かめたのは PX-Q3U4・PX-W3U4・MLT5 系のみ）。WebTS で書いた周り（USB の許可、受信機の割り当て）は試験で確かめた。**動いた・動かなかったの報告を募集している**（動作報告をオンにすると動作ログが届く仕組みがある。beta 版は既定で有効、本番は未確認機種の接続時の案内または設定からオンにできます）。
