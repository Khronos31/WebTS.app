...
**アンインストールが必要とまでは言えない。**あとで確認したところ mirakc は
インストールされたまま有効で、動いてはいなかった。効いたのは既定の割り当てを
持つアプリがデバイスを取っていたことだと考えられる。既定の解除
（設定 → アプリ → 既定で開く）や強制停止で足りる可能性が高いが、**試していない。**
...
- **Android でのフレーム落ちの数値。**体感では問題ないが、TS ドロップと
  A/V ずれを読んでいない。
- **Android と Linux での長時間運転**、切断・再接続。
- **macOS、ChromeOS。**
