...
| Android | 他のチューナーアプリ（mirakc 等） | **競合する。**アンインストールするまで開けない |

**Android は USB デバイスをアプリ単位で排他的に掴む。**他のチューナーアプリが
デバイスへの接続を保持していると、ブラウザからは claim できない。実測では
`Khronos31/dtv-android` の mirakc を入れたままだと開けず、外すと通った。
