...
**入口 39,991 に対して出口 38,263 で、1,728 packet（4.3%）少ない。**
PAT/PMT/ECM が揃う前のパケットが落ちているという読みが自然だが、
**確かめていない。**`flush()` のあとにもう一度 `get()` して数えているので、
単に内部に残っているだけではない。`set_strip(0)` なので明示的な破棄でもない。
映像として繋ぐ前にここは詰める必要がある。
...
- **これはまだ映像ではない。**分離（PMT から映像・音声 PID を引く）と
  MPEG-2 復号、AAC 復号、描画が残っている。
- 30分連続は未実施。最長5秒である。
