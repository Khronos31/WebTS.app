...
当初は再配布の可否を考慮して webts.app からは配らず、利用者がプレクスのドライバを取得して
ページへ渡す方式を採っていた。ブラウザからドライバを直接取得することはできないため
（`plex-net.co.jp` は CORS ヘッダを返さず、静的ホスティングにプロキシもない）、利用者が
手動で ZIP/.sys を与えてブラウザ内で抽出し、取り出した 2169 バイトを端末の IndexedDB に
保存していた。

その後、利用者の利便性を優先して方針を改め、WebTS.app の配布サーバから
`/firmware/it930x-firmware.bin` として直接配布する方式へ移行した。リポジトリには含めず、
デプロイ時に GitHub Actions（`scripts/fetch-firmware.mjs`）がプレクスの公式ドライバを
ダウンロードしてファームウェアを取り出し、配信物に配置する。アプリは視聴や走査を
開始するたびに配布サーバから取得し、端末の IndexedDB への永続化や利用者によるドライバ
提供経路は廃止した（以前 IndexedDB に保存されていたデータは削除する）。

取り出し時の照合手順自体はどちらの方式でも共通であり、表の3段階すべてがそのまま有効である。
...
**IT930x はコールドブート毎にファームウェアを要求する**。視聴や走査を始めるたびにアプリが配布サーバから取得するのはこのためである（以前の方式ではこれに備えて取り出した 2169 バイトを IndexedDB に置いていた）。
