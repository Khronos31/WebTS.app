...
// It loads an opt-in generated module that installs a fake navigator.usb and
// runs exactly one scenario. No real USB, requestDevice, firmware, mode, tune,
// TS, or B25 path is reachable from here, and the Worker is terminated by the
// caller after one scenario because an unmet expectation parks libusb state.

interface OwnershipModule {
  ccall(name: string, returnType: 'number', argTypes: string[], args: number[],
    options?: { async: true }): Promise<number>;
...
async function execute(moduleUrl: string, scenario: number) {
  ...
  try {
    ...
    const view = new DataView(module.HEAPU8.buffer, module.HEAPU8.byteOffset + pointer,
      REPORT_WORDS * 4);
    const report: Record<string, unknown> = { diagnostic };
    for (let index = 0; index < REPORT_WORDS; index += 1) {
...
function failed(diagnostic: 'WORKER_FAILED' | 'INVALID_RESULT') {
  const report: Record<string, unknown> = { diagnostic };
  for (const field of FIELDS) report[field] = null;
  report.physicalAbortProven = false;
  report.realUsbUsed = false;
  return report;
}
